...
export type AvatarProvider = 'gravatar' | 'cravatar';

// 根据指定 provider 获取头像 URL
export function getAvatarUrl(email: string, provider: AvatarProvider = 'gravatar', size = 80): string {
  if (provider === 'cravatar') return getCravatarUrl(email, size);
...
export function getUserAvatarUrl(
  ...
  return getAvatarUrl(user.email, provider, size);
}
